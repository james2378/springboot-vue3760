<template>
	<div class="diy_details page_journal_articles" id="journal_articles_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_journal_articles'>
							<div_journal_articles :obj="obj"></div_journal_articles>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_journal_articles from "@/components/diy/div_journal_articles.vue";
	import mixin from "@/mixins/page.js";
	export default {
		mixins:[mixin],
		components:{
			div_journal_articles
		},
		data(){
			return{
		...
		methods:{
			...
			get_collect(obj) {
				...
				var query = {
					...
					source_field: "journal_articles_id",
					source_id: obj["journal_articles_id"],
					user_id
				};
		 ...
		 /****
			...
			change_collect() {
				...
				var query = {
					source_table: "journal_articles",
					source_field: "journal_articles_id",
					source_id: this.obj["journal_articles_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/journal_articles/set?journal_articles_id=' + _this.obj["journal_articles_id"], {
								praise_len
							}, (res) => {
								if(res.result){
