		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','name_of_expert') || $check_field('add','name_of_expert') || $check_field('set','name_of_expert')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="专家姓名" prop="name_of_expert">
					<el-input id="name_of_expert" v-model="form_sub['name_of_expert']" placeholder="请输入专家姓名"
							  v-if="user_group === '管理员' || (form_sub['expert_user_id'] && $check_field('set','name_of_expert')) || (!form_sub['expert_user_id'] && $check_field('add','name_of_expert'))"></el-input>
					<div v-else-if="$check_field('get','name_of_expert')">{{form_sub['name_of_expert']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','gender') || $check_field('add','gender') || $check_field('set','gender')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="性别" prop="gender">
					<el-select id="gender" v-model="form_sub['gender']"
						v-if="user_group === '管理员' || (form_sub['expert_user_id'] && $check_field('set','gender')) || (!form_sub['expert_user_id'] && $check_field('add','gender'))">
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 提交附加信息
			submit_sub(form_sub){
				if(form_sub["expert_user_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?expert_user_id=" + form_sub["expert_user_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/expert_user/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/expert_user/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/expert_user/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/expert_user/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/expert_user/view','get');
